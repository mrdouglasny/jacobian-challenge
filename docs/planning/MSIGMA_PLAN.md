...
- [~] Mσ.3. concrete `pullbackInvolution : HolomorphicOneForm →ₗ[ℂ] HolomorphicOneForm`   status: in_progress   deps: [Mσ.2]   note: per `docs/Msigma3-codex-plan.md`. DONE (Codex 41b0a4a/558ab3f/324d42a, green): `pullbackInvolutionChartRep` (A_q), `pullbackInvolutionDerivFactor` (B_q), `pullbackInvolutionCoeff`, the of_mem/of_not_mem unfolds, obligation (3) `…_isZeroOffChartTarget`, and the projX simplifications `pullbackInvolutionChartRep_projX`/`…DerivFactor_projX`/`…Coeff_projX` (the Mσ.4 consumer). REMAINING: obligation (1) `IsHolomorphicOneFormCoeff` (analyticOn each target), obligation (2) `SatisfiesCotangentCocycle` (the heavy ~120–200 LOC fderiv chain-rule), then package `pullbackInvolution` linear map. (NB pre-existing orphan `omegaDx_analyticAt` sorry is unrelated.) Re-dispatched to Codex.
- [~] Mσ.4. `sigma_invariant_form_eq_zero (η) (hinv : pullbackInvolution H η = η) : η = 0`   status: in_progress   deps: [Mσ.3]   note: direct-Liouville core (handoff §Mσ.4). 4 steps: (1) σ-invariance ⇒ coeff single-valued `c(x)`; (2) `c` entire [building block BANKED: `differentiable_of_analyticAt_off_roots` e3d407f — analytic off {f=0}+continuous ⇒ entire, via P2+removable-sing]; (3) `c=O(1/x²)` at the two ∞ points; (4) Liouville ⇒ c≡0 [BANKED: `eq_zero_of_differentiable_tendsto_zero_cocompact` cd7ae6c — entire+→0 at ∞ ⇒ 0] ⇒ η=0 (`ext_of_coeff`). REMAINING (gated on Mσ.3 pullback API): steps 1 (single-valued c from `pullbackInvolutionCoeff_projX`) + 3 (∞-chart growth). I take this once Mσ.3 lands.
