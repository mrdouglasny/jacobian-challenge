...
## Fifth pass (2026-06-12, TOPO lane) — B1 DELIVERED; the mountain bypassed

Status: **G3, G4, G5 and the G6 sharpening are DONE and kernel-verified
(standard-3).** Files: `SpokeAlgebra.lean` (spoked-class algebra),
`CellGeneration.lean` (the binary-split engine), `LassoGeneration.lean`
(circle-ization), `IsolatingLasso.lean` (meridian conjugacy + the
normal-closure headline), plus `halfPlaneHomeoGT` and
`puncturedBallHomeo` in the toolkit file.

**What was proved.**
* `fromPath_mem_of_cellSpokes_subset` — the induction engine, in
  abstract-subgroup membership form over ALL basepoints and spokes
  simultaneously (the strengthening the second-pass G4 notes called
  for).  Every transport (half-plane sides, rotation for constant-re
  configurations, rebasing) is a `Subgroup.comap` along a π₁ hom; the
  generating family is closed under the pullbacks because spokes absorb
  every basepoint correction.
* `closure_circleLassos_eq_top` — π₁(ℂ ∖ T, x₀) is generated, under
  PLAIN `Subgroup.closure`, by the explicit spoked isolating circle
  lassos (`circleInPunctured`, the `exists_winding_dual_loops` loop
  shape).
* `normalClosure_isolatingLassos_eq_top` — **one isolating lasso per
  puncture** (any choice) normally generates.  Meridian conjugacy
  (`isolatingLasso_conj`) is elementary for isolating lassos: both
  circles live in a common punctured ball, presented as a once-punctured
  plane by `puncturedBallHomeo`, where the winding-unit argument pins
  both to the same power of the cell generator.

**The fourth-pass "mountain" is bypassed for every concrete B1 form.**
The filling-kernel / 2D homotopy telescope is needed ONLY for the
arbitrary-`PuncturedCellSystem` headline (cells with abstractly
homeomorphic presentations whose lassos are pinned by winding alone) —
that statement is true (via Jordan–Schoenflies-grade input) but is now
Mathlib-grade generality, not on the program's critical path.

**Remaining in this cluster:** B2 (freeness on these generators) only,
pending the B3 consumer answer on issue #171.  The δ-winding matrix
(`windingHom_lasso_self`/`_ne`) + `normalClosure_isolatingLassos_eq_top`
already deliver generators-with-identified-winding; H₁-level
consequences (H₁(ℂ ∖ T) generated by the `T.card` lasso images) follow
by abelianization without any further topology.
