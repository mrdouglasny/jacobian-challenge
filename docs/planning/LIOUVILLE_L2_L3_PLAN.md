...
- [~] L2a. entire extension `g` (`Differentiable ℂ`) — branch-point + seam regularity   status: blocked   deps: [L0, Mσ]   note: BLOCKED ON Mσ ANTI-INVARIANCE. Architecture pinned + Mathlib names verified (`docs/planning/L2_DIFFERENTIABLE_G_BLUEPRINT.md`); Riemann removable singularity, 4 pieces. P2 isolated roots DONE (1fd792e). Sheet-swap helper DONE (`liouvilleProjXNumerator_eq_of_neg_coeff_neg_branch`, 4a02267). P1 now reduces to the SINGLE goal `⊢ form.coeff qSame z = −form.coeff qOpp z` = hyperelliptic anti-invariance `ι*ω = −ω` for an arbitrary holomorphic form. That is the **Mσ workstream**: `Involution.lean` (σ on surface) DONE; `docs/Msigma-codex-handoff.md` + `docs/genus-L2-execution-roadmap.md` prescribe the direct-Liouville route (no quotient); but `AntiInvariance.lean` is a 1-sorry stub (`omegaDx_analyticAt`). So L2 ≠ assembly — it is GATED on completing Mσ (substantial). P3 (branch ContinuousAt) also still open but downstream of P1. **AWAITING MRD: pursue Mσ now vs redirect to a cheaper axiom.**
...
- [ ] D. retire both axioms; `#print axioms` verify; reconcile counts 55→53; PR   status: todo   deps: [L2, L3]
