...
- [~] L2a. entire extension `g` (`Differentiable ℂ`) — branch-point + seam regularity   status: in_progress   deps: [L0]   note: ARCHITECTURE PINNED + Mathlib names VERIFIED — see `docs/planning/L2_DIFFERENTIABLE_G_BLUEPRINT.md`. Route = Riemann removable singularity (`Complex.analyticAt_of_differentiable_on_punctured_nhds_of_continuousAt`). 4 pieces: P1 non-branch `AnalyticAt` (EventuallyEq via overlap+anti-invariance), P2 isolated roots [DONE — `eventually_eval_ne_zero_nhdsWithin`, 1fd792e], P3 branch `ContinuousAt` (ramified-pullback limit — HARD), P4 assemble. OPEN: P1 cross-sheet anti-invariance + P3.
