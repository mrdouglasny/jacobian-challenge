...
**Blocked by:** `PlaneCurve`, `AX_PlaneCurveAffine_nonempty`
...
**Why it's an axiom right now:** Stub forced by the axiomatic `PlaneCurve` type at `PlaneCurve.lean:161`. The affine patch is already inhabited by axiom `AX_PlaneCurveAffine_nonempty` at `PlaneCurve.lean:103–106`; the unified `PlaneCurve H` is its compactification, so the affine point maps to a point of `PlaneCurve H` via the canonical inclusion-into-central-chart map.
...
3. The required `AX_PlaneCurveAffine_nonempty H : Nonempty (PlaneCurveAffine H)` is at `Jacobians/ProjectiveCurve/PlaneCurve.lean:103` and already declared `[instance]` at line 106, so even `inferInstance` may close the goal once `centralCoe` is available.
