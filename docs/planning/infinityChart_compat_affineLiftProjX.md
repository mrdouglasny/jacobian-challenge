...
1. **Prerequisites.** Both `infinityChart` and `infinityInverseMap` must be real `noncomputable def`s (per their recipes). Available API after those discharges: `infinityInverseMap_analyticOn`, `infinityInverseMap_x_eq`, `infinityInverseMap_y_eq`, `someRadius`, plus the simp lemmas from the `infinityChart` recipe (`infinityChart_source_eq`, `infinityChart_target_eq` if exposed). The pattern lemma `OpenPartialHomeomorph.lift_openEmbedding_apply` is at `.lake/packages/mathlib/Mathlib/Topology/OpenPartialHomeomorph/Constructions.lean:388`; `lift_openEmbedding_source` at `:394`.
...
2. **Compute the transition explicitly.** The forward composite map `t ↦ (...) ↦ x` works out (using `OnePoint.isOpenEmbedding_coe` and `lift_openEmbedding_apply`) to
   ...
   ```
   on its source (everywhere a punctured disk + the `target` of `affineChartProjX p hpY`). Use `infinityInverseMap_x_eq` (recipe of `infinityInverseMap`, Step 5) to rewrite this as `c⁻¹ · t^{−2} · x̂(t)`, where `x̂` is analytic on `‖t‖ < someRadius H h` and `x̂(0) = 1`. Prove this via a sequence of `simp`s using `OpenPartialHomeomorph.trans_apply`, `lift_openEmbedding_apply`, `affineChartProjX` unfolding (which is the projection `(x, y) ↦ x`, see `OddAtlas/AffineChart.lean:163`).
