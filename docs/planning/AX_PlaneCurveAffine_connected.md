...
4. **Affine patch as a complement of finite points.** Show that `PlaneCurveAffine H` is exactly the projective curve minus the set of points at infinity (the intersection with the line $Z = 0$). By Bézout, this intersection is a finite set of points. (Support with `isClosed_carrier` at `PlaneCurve.lean:82` and `AX_PlaneCurveAffine_nonempty` at `PlaneCurve.lean:103` — now a **theorem**, PR #92, not an axiom).
