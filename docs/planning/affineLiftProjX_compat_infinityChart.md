...
- If the statement needs to change shape (e.g. the `lift_openEmbedding` API moves in a Mathlib bump), do not silently rewrite — consumer at `OddAtlas.lean:120` and the precise `OpenPartialHomeomorph.lift_openEmbedding` API at `.lake/packages/mathlib/Mathlib/Topology/OpenPartialHomeomorph/Constructions.lean:347` are load-bearing.
