...
## Update 2026-06-12 (TOPO lane, post-B1)

B1 landed in full (`docs/planning/B1_GENERATION_ROUTE.md`, fifth pass):
`normalClosure_isolatingLassos_eq_top` gives, for the PUNCTURED PLANE,
finite normal generation of π₁ by one explicit isolating lasso per
puncture — without any good cover.  Consequences for this blocker:

* **GC-2 re-scoping recommended.** Where the consumer chain reduces to
  slit-sheet / punctured-plane pieces (the TR-DISC route), H₁
  finiteness now follows from B1-sharp by abelianization: H₁(ℂ ∖ T) is
  generated by the `T.card` lasso images.  The good-cover existence
  question (GC-1) remains the gap only for results stated on the
  COMPACT surface itself with no slit-sheet reduction.
* **GC-1 status unchanged**: the convex-ball route stays closed at the
  Mathlib pin (no Riemannian exponential map / totally normal
  neighborhoods); routes 1–3 of the original analysis stand.  The new
  `puncturedBallHomeo` + `CellGeneration` machinery does not by itself
  produce simply connected chart covers with path-connected pairwise
  intersections on a compact surface.
