...
| **E3** per-arc constructor | chart-disk subdivision of an `IsSmoothPath`; one-disk weak solution `exp(ψ·log((z−b)/(z−a)))` ≡ 1 off the disk (Forster 20.5); product/fold over the subdivision and the chain | H | **PROVEN COMPLETE** (`AbelSubsetEngineArc.lean`): `SegTube` planar layer + master `∂̄`-identity; `exists_arcWeakSolution_avoiding` (per-arc lift, avoid-set tameness); `ArcWeakSolution.exists_mul` (binary fold + tameness propagation); `exists_subdivision` (Lebesgue); `exists_arcWeakSolution_dipoleList` (master fold); **`exists_arcWeakSolution_boundary`** — every chain, unconditional, standard-3 |
| **E4** pairing identity | `pairOmega 𝔇 σ_F α = π·∫_c α` for the E3 datum — the honest constant in the Lebesgue-area chart-coefficient convention is `π`, NOT `2πi` (`dz̄∧dz = 2i·dA`: Forster's `∫∫σ∧α = 2πi∫_c α` divided by `2i`; consistent with `resNormalization = −π⁻¹`). The E2 field's `2πi·period` is only ever consumed at `period = 0`, so the `π`-identity discharges it | H | **PROVEN COMPLETE (E4 session)**: (a) planar atom `integral_slitLog_dbar_mul` (`SlitLogIntegral.lean`: moving-pole rep `slitLogRatio_eq_integral` + Fubini + `cauchyPompeiu_area` per slice); (b) `intervalIntegral_form_eq_segPeriod` (`SegmentPeriod.lean`: general-α chart-frame cancellation + Morera primitive endpoint diffs); (c) pairing accumulator through the fold (`pairOmega_sigma_add`, tagged dipole lists, `exists_arcWeakSolution_dipoleListChart`, `exists_arcWeakSolution_boundary_pairing`). **`exists_meromorphic_of_zeroPeriodChain'` is UNCONDITIONAL** |
...
| **E6** adapter (root-side, separate Bridge file/PR) | E6a open-path developing↔lineIntegral; E6b divisor faithfulness; assemble `ZeroPeriodChainSolvability` | M/H | **PROVEN COMPLETE (2026-06-12, `Bridge/AbelEngineAdapter.lean`)** — E6a consumed from #216 + the per-arc comparison-loop lattice absorption (no fresh open-path brick needed); E6b via the port identity theorem `orderW_ne_top_of_exists` + Layer3 order dictionaries; `abel_subset` standard-3 + `AX_PeriodCycleBasis`; `AX_AbelTheorem` FLIPPED (split: remainder `AX_AbelSupset`) |
...
4. ~~E4~~ (E4 session: planar atom + segment-period brick + fold accumulator;
   `exists_meromorphic_of_zeroPeriodChain'` unconditional, standard-3).
