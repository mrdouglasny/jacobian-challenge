...
| **E4** pairing identity | `pairOmega 𝔇 σ_F α = 2πi·∫_c α` for the E3 datum (Forster 20.3/20.5: planar change of variables; contour↔Laurent brick `resAt_eq_planarCoeff_neg_one` landed in `TailFrameWitness.lean`) | H | open — THE single remaining engine obligation, pinned by `exists_meromorphic_of_zeroPeriodChain_of_pairing`. Sub-bricks: (a) per-arc planar value `pairOmega (lift H·∂̄lift ψ) α = 2πi·∫_[za,zb] α̃` (Stokes on the tube annulus + slit-log jump); (b) segment-vs-arc-piece period equality (holomorphic form, disk homotopy); (c) additivity through the fold (needs a pairing accumulator carried alongside the fold, or a σ-decomposition-exposing variant of the master fold) |
