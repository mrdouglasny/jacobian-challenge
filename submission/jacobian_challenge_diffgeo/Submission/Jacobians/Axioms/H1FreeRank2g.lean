...
from `AX_PeriodCycleBasis`, which supplies a ℤ-basis of `H_1(X, ℤ)`
...
sketches in `Jacobians/Axioms/PeriodCycleBasis.lean`.
...
`2 * genus X`. Derived from `AX_PeriodCycleBasis`, which supplies a
