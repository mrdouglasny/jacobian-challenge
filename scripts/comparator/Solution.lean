...
/-!
Comparator solution: delegates to `Jacobians.Challenge` proved declarations.
Passes `comparator config-buzzard.json` once both challenge-critical axioms
(`AX_PeriodCycleBasis`, `AX_AbelTheorem`) are discharged — every listed
theorem will then print only `[propext, Classical.choice, Quot.sound]`.
-/
...
theorem ofCurve_contMDiff (P : X) :
    ContMDiff 𝓘(ℂ) 𝓘(ℂ, Fin (genus X) → ℂ) ω (Jacobian.ofCurve P) :=
  Jacobian.ofCurve_contMDiff P

theorem ofCurve_self (P : X) :
    Jacobian.ofCurve P P = 0 :=
  Jacobian.ofCurve_self P
