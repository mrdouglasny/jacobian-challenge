...
-- construction. We provide a variant `chartedSpaceOfAtPreimage` built from `chartAtPreimage`
...
with charts explicitly `(chartAtPreimage hf _).symm`. This construction is definitionally
transparent: `chartAt y = (chartAtPreimage hf (Classical.choose (hs y))).symm`.
