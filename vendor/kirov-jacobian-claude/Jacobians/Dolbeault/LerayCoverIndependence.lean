...
import Jacobians.Dolbeault.CechRefinementCommon
...
/-- **`exists_cechModel` transported along cover-independence.** If any Leray cover `𝔙` admits a
chart-disk model `d` and `Coboundaries d` (with its `leray` acyclicity field) and a comparison
`𝔙.cechH1 D ≃ₗ[ℂ] c.supH1`, then any other Leray cover `𝔘` also admits a model and comparison.
We construct the comparison for `𝔘` by composing the cover-independence isomorphism
`cechH1_equiv_of_leray 𝔘 𝔙` with the given comparison for `𝔙`. -/
theorem exists_cechModel_of_leray_comparison (𝔘 𝔙 : FiniteCover X)
    (hU : 𝔘.IsLeray) (hV : 𝔙.IsLeray) (D : Divisor X)
    (d : DiskOverlapData) (c : Coboundaries d) (e : 𝔙.cechH1 D ≃ₗ[ℂ] c.supH1) :
    ∃ (d' : DiskOverlapData) (c' : Coboundaries d'), Nonempty (𝔘.cechH1 D ≃ₗ[ℂ] c'.supH1) :=
  let iso := FiniteCover.cechH1_equiv_of_leray 𝔘 𝔙 hU hV D
  ⟨d, c, ⟨iso.trans e⟩⟩
