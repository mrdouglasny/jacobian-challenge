...
theorem Path.exists_ball_chart_subdivision
    ...
    ∃ (t : ℕ → I) (φ : ℕ → OpenPartialHomeomorph Y ℂ)
      ...
      (∀ i, (φ i).source ⊆ (chartAt ℂ ((φ i).symm (centers i))).source) ∧
      (∀ i, ∀ y ∈ (φ i).source, (φ i) y = (chartAt ℂ ((φ i).symm (centers i))) y) ∧
      (∀ i, (φ i).target ⊆ (chartAt ℂ ((φ i).symm (centers i))).target) := by
  -- For each `y : I`, choose a ball-restricted chart at `γ y`.
  choose r hr_pos φ hy_src hφ_target _hφ_eq _hφ_sub_src _hφ_sub_tgt using
  ...
  have h_symm (i : ℕ) : (φ (yidx i)).symm ((chartAt ℂ (γ (yidx i))) (γ (yidx i))) = γ (yidx i) := by
    have h1 : (φ (yidx i)) (γ (yidx i)) = (chartAt ℂ (γ (yidx i))) (γ (yidx i)) := by
      exact congr_fun (_hφ_eq (yidx i)) (γ (yidx i))
    have h2 : (φ (yidx i)).symm ((φ (yidx i)) (γ (yidx i))) = γ (yidx i) := by
      exact (φ (yidx i)).left_inv (hy_src (yidx i))
    rw [← h1]
    exact h2
  refine ⟨t, fun i => φ (yidx i), fun i => r (yidx i),
         fun i => (chartAt ℂ (γ (yidx i))) (γ (yidx i)),
         ht0, ht_mono, ⟨N, htN⟩, ?_, ?_, ?_, ?_, ?_, ?_⟩
  · intro i; exact hr_pos (yidx i)
  · intro i; exact hφ_target (yidx i)
  · intro i s hs; exact hyidx i hs
  · intro i; rw [h_symm i]; exact _hφ_sub_src (yidx i)
  · intro i y _hy; rw [h_symm i]; exact congr_fun (_hφ_eq (yidx i)) y
  · intro i; rw [h_symm i]; exact _hφ_sub_tgt (yidx i)
