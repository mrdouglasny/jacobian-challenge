...
- If the "degree-1 holomorphic map between compact connected Riemann surfaces is a biholomorphism" step (step 5) needs `AX_BranchLocus` and that axiom is not yet usable in a form sufficient for this conclusion, either route through it or temporarily introduce a small intermediate axiom `AX_degree_one_iff_iso` rather than fudging step 5.
