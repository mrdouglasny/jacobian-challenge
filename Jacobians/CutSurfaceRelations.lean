...
/-- Existence of a symplectic homology basis of closed smooth loops on X. -/
lemma exists_symplectic_homology_basis (X : Type*) [TopologicalSpace X] [T2Space X] [CompactSpace X]
    [ConnectedSpace X] [Nonempty X] [ChartedSpace ℂ X] [IsManifold 𝓘(ℂ) ω X] :
    ∃ (loop : Fin (2 * genus X) → (ℝ → X)) (h_cl : ∀ k, IsClosedSmoothLoop (loop k)),
      closedLoopPeriods X ⊆ Submodule.span ℤ (Set.range (fun k => periodVec (loop k))) := sorry

/-- Existence of pulled-back holomorphic forms, primitives, and boundary relations on the cut box. -/
lemma exists_pulledback_forms_and_primitives (X : Type*) [TopologicalSpace X] [T2Space X] [CompactSpace X]
    [ConnectedSpace X] [Nonempty X] [ChartedSpace ℂ X] [IsManifold 𝓘(ℂ) ω X]
    (loop : Fin (2 * genus X) → (ℝ → X)) :
    ∃ (U : Set ℂ) (hbox : wCLM '' (Set.Icc 0 1 ×ˢ Set.Icc 0 1) ⊆ U)
      (h : Fin (genus X) → ℂ → ℂ) (F : Fin (genus X) → ℂ → ℂ)
      (hh : ∀ i, ∀ z ∈ U, HasDerivAt (h i) (deriv (h i) z) z)
      (hF : ∀ i, ∀ z ∈ U, HasDerivAt (F i) (h i z) z),
      (∀ i j, ((aPeriodBlock loop)ᵀ * bPeriodBlock loop - (bPeriodBlock loop)ᵀ * aPeriodBlock loop) i j
        = rectBoundaryIntegral (fun z => F i z * h j z)) ∧
      (∀ i j, ((aPeriodBlock loop)ᵀ * (bPeriodBlock loop).map (starRingEnd ℂ)
        - (bPeriodBlock loop)ᵀ * (aPeriodBlock loop).map (starRingEnd ℂ)) i j
        = - boundaryForm (h j) (F i)) ∧
      (∀ v : Fin (genus X) → ℂ, v ≠ 0 → ∃ p ∈ Set.Ioo (0:ℝ) 1 ×ˢ Set.Ioo (0:ℝ) 1, (∑ j, v j * h j (wCLM p)) ≠ 0) := sorry
...
theorem exists_cutSurface (X : Type*) [TopologicalSpace X] [T2Space X] [CompactSpace X]
    [ConnectedSpace X] [Nonempty X] [ChartedSpace ℂ X] [IsManifold 𝓘(ℂ) ω X] :
    Nonempty (CutSurface X) := by
  obtain ⟨loop, h_cl, h_gen⟩ := exists_symplectic_homology_basis X
  obtain ⟨U, hbox, h, F, hh, hF, h_R1, h_R2, h_nondeg⟩ := exists_pulledback_forms_and_primitives X loop
  exact ⟨{
    loop := loop
    loop_closed := h_cl
    generates := h_gen
    ...
    hh := hh
    hF := hF
    boundaryWord_R1 := h_R1
    boundaryWord_R2 := h_R2
    nondeg := h_nondeg
  }⟩
