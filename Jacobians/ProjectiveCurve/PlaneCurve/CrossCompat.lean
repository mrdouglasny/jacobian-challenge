...
/-- **Axiom (NOT VERIFIED — sound as transitions are rational/analytic).**
The transition maps between any two charts in the atlas on `PlaneCurve H` are complex analytic. -/
axiom chartAt_compat (H : PlaneCurveData) (q q' : PlaneCurve H) :
    ContDiffOn ℂ ω
...
noncomputable instance PlaneCurve.instChartedSpace (H : PlaneCurveData) :
  ...
  _root_.Jacobians.ProjectiveCurve.instChartedSpace H
