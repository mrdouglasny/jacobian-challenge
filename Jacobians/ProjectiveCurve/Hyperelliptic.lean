...
private noncomputable def Hyperelliptic.carrierOf_genus (H : HyperellipticData)
    (d : Decidable (Odd H.f.natDegree)) : ℕ :=
  @Jacobians.RiemannSurface.genus (Hyperelliptic.carrierOf H d)
    (Hyperelliptic.topologicalSpaceOf H d)
    (Hyperelliptic.t2SpaceOf H d)
    (Hyperelliptic.compactSpaceOf H d)
    (Hyperelliptic.connectedSpaceOf H d)
    (Hyperelliptic.chartedSpaceOf H d)
    (Hyperelliptic.isManifoldOf H d)

/-- The genus of `y² = f(x)` matches the combinatorial formula `⌊(deg f - 1) / 2⌋`. -/
theorem AX_Hyperelliptic_genus (H : HyperellipticData) :
    Jacobians.RiemannSurface.genus (Hyperelliptic H) = H.genus := by
  change Hyperelliptic.carrierOf_genus H (Nat.instDecidablePredOdd H.f.natDegree) = H.genus
  cases Nat.instDecidablePredOdd H.f.natDegree with
  | isTrue h =>
    change Jacobians.RiemannSurface.genus (HyperellipticOdd H h) = H.genus
    unfold HyperellipticData.genus
    exact Jacobians.Extensions.Hyperelliptic.genus_HyperellipticOdd_eq H h
  | isFalse h =>
    haveI : Fact (¬ Odd H.f.natDegree) := ⟨h⟩
    change Jacobians.RiemannSurface.genus (HyperellipticEvenProj H) = H.genus
    rw [Jacobians.Extensions.HyperellipticEven.genus_HyperellipticEven_eq H]
    unfold HyperellipticData.genus
