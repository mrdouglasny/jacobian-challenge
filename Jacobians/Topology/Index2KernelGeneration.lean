...
/-! ## Surjection transports generation (the connective bridge)

The piece that composes Fact (1) with the **puncture-fill van Kampen
surjection** (Fact (2)): a surjective homomorphism carries a generating set of
the source onto a generating set of the target, provided each source generator
lands in the subgroup generated by the proposed target generators. -/

variable {A B : Type*} [Group A] [Group B]

/-- **Surjection transports generation.** If `q : A →* B` is surjective, `R`
generates `A`, and every `q r` (for `r ∈ R`) lies in `closure L`, then `L`
generates `B`.

In the branched-cover application `q = π_*` is the puncture-fill surjection
`π₁(X∖T) ↠ π₁(X)`, `R` is the Schreier set generating `φ.ker = π₁(X∖T)` (Fact
(1) above, via the covering identification), and `L` is the family of
branch-cut loops. The hypothesis `hmap` is exactly "each Schreier generator
projects into the branch-cut-loop subgroup" — the meridian generators `t * t`
project to the identity (they bound a disk after the puncture is filled),
while the cycle generators `t * s` / `s * t⁻¹` project to the branch-cut
loops. -/
theorem closure_eq_top_of_surjective {q : A →* B} (hq : Function.Surjective q)
    {R : Set A} (hR : closure R = ⊤) {L : Set B}
    (hmap : ∀ r ∈ R, q r ∈ closure L) : closure L = ⊤ := by
  rw [eq_top_iff]
  rintro b -
  obtain ⟨a, rfl⟩ := hq b
  have ha : a ∈ closure R := hR ▸ mem_top a
  induction ha using closure_induction with
  | mem x hx => exact hmap x hx
