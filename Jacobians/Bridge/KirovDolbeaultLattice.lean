...
This file defines the coordinate and basis bridge between our Riemann surface
formalization and the Kirov-Dolbeault port:
- `genus_eq_kirovGenus`
- `latticeBridge`
-/
...
omit [Nonempty X] in
/-- The geometric genus of `X` is equal to the Kirov genus. -/
...
/-- The linear map transferring Kirov's period vectors to our basis coordinates. -/
def latticeBridge (Y : Type*) [TopologicalSpace Y] [T2Space Y] [CompactSpace Y]
  ...
  map_smul' c x := by
    ...
    have h1 : (fun i => ((ambientIso Y).symm (bridgeKDFormEquiv (jacobianBasis Y j))) i * (c * x i)) =
        (fun i => c * (((ambientIso Y).symm (bridgeKDFormEquiv (jacobianBasis Y j))) i * x i)) := by
      funext i; ring
    simp_rw [h1]
    rw [Finset.mul_sum]

/-- The adjoint (transpose) identity relating ambientTrace and ambientPullbackJac. -/
...
/-- Wrapping Kirov's line integral as a ℂ-linear map. -/
noncomputable def lineIntegralLinearMap {Y : Type*} [TopologicalSpace Y] [T2Space Y] [CompactSpace Y]
    [ConnectedSpace Y] [ChartedSpace ℂ Y] [IsManifold 𝓘(ℂ) ω Y] [Nonempty Y]
    (γ : ℝ → Y) (hγ : IsClosedSmoothLoop γ) :
    HolomorphicOneForms Y →ₗ[ℂ] ℂ where
...
/-- The period vector maps under `latticeBridge` to Kirov's period vector values. -/
theorem latticeBridge_periodVec {Y : Type*} [TopologicalSpace Y] [T2Space Y] [CompactSpace Y]
    [ConnectedSpace Y] [ChartedSpace ℂ Y] [IsManifold 𝓘(ℂ) ω Y] [Nonempty Y]
    (γ : ℝ → Y) (hγ : IsClosedSmoothLoop γ) (j : Fin (genus Y)) :
    latticeBridge Y (periodVec γ) j = lineIntegral (bridgeKDFormEquiv (jacobianBasis Y j)) γ := by
  ...
  have h_eq (i : Fin (kirovGenus Y)) : lineIntegral (ambientIso Y (Pi.basisFun ℂ (Fin (kirovGenus Y)) i)) γ =
      (lineIntegralLinearMap γ hγ) (ambientIso Y (Pi.basisFun ℂ (Fin (kirovGenus Y)) i)) := rfl
  simp_rw [h_eq]
  have h_sum : ∑ i, c i * (lineIntegralLinearMap γ hγ) (ambientIso Y (Pi.basisFun ℂ (Fin (kirovGenus Y)) i)) =
      (lineIntegralLinearMap γ hγ) (∑ i, c i • ambientIso Y (Pi.basisFun ℂ (Fin (kirovGenus Y)) i)) := by
    rw [map_sum]
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [map_smul, smul_eq_mul, mul_comm]
  rw [h_sum]
  set η := bridgeKDFormEquiv (jacobianBasis Y j)
  have h_basis : ∑ i, ((ambientIso Y).symm η) i • ambientIso Y (Pi.basisFun ℂ (Fin (kirovGenus Y)) i) = η := by
    simp_rw [← map_smul]
    rw [← map_sum]
...
/-- The pairing of a holomorphic 1-form with a Kirov period vector, expressed as a ℂ-linear map. -/
...
/-- The evaluation of the dual basis equivalence on `latticeBridge Y w` matches `pairingWithW w`. -/
theorem eY_symm_latticeBridge {Y : Type*} [TopologicalSpace Y] [T2Space Y] [CompactSpace Y]
    [ConnectedSpace Y] [ChartedSpace ℂ Y] [IsManifold 𝓘(ℂ) ω Y] [Nonempty Y]
    (w : Fin (kirovGenus Y) → ℂ) :
...
/-- The inverse linear map of `latticeBridge Y`. -/
noncomputable def latticeBridgeInv (Y : Type*) [TopologicalSpace Y] [T2Space Y] [CompactSpace Y]
    [ConnectedSpace Y] [ChartedSpace ℂ Y] [IsManifold 𝓘(ℂ) ω Y] [Nonempty Y] :
...
theorem latticeBridgeInv_left_inverse {Y : Type*} [TopologicalSpace Y] [T2Space Y] [CompactSpace Y]
    [ConnectedSpace Y] [ChartedSpace ℂ Y] [IsManifold 𝓘(ℂ) ω Y] [Nonempty Y]
    (w : Fin (kirovGenus Y) → ℂ) :
    latticeBridgeInv Y (latticeBridge Y w) = w := by
  ext k
  dsimp [latticeBridgeInv]
  have h_pair : ((jacobianBasis Y).dualBasis.equivFun.symm (latticeBridge Y w)) = pairingWithW w :=
    eY_symm_latticeBridge w
  rw [h_pair]
  unfold pairingWithW
  simp only [LinearMap.coe_mk, AddHom.coe_mk]
  have h_trans : (ambientIso Y).symm (bridgeKDFormEquiv ((bridgeKDFormEquiv (X := Y)).symm (ambientIso Y (Pi.basisFun ℂ (Fin (kirovGenus Y)) k)))) = Pi.basisFun ℂ (Fin (kirovGenus Y)) k := by
...
theorem latticeBridgeInv_right_inverse {Y : Type*} [TopologicalSpace Y] [T2Space Y] [CompactSpace Y]
    [ConnectedSpace Y] [ChartedSpace ℂ Y] [IsManifold 𝓘(ℂ) ω Y] [Nonempty Y]
    (v : Fin (genus Y) → ℂ) :
  ...
  have h_eq : (jacobianBasis Y).dualBasis.equivFun.symm (latticeBridge Y (latticeBridgeInv Y v)) =
    ...
    have h_eval : ∑ i, ((ambientIso Y).symm η) i * (latticeBridgeInv Y v) i =
        ((jacobianBasis Y).dualBasis.equivFun.symm v) (bridgeKDFormEquiv.symm (ambientIso Y (∑ i, ((ambientIso Y).symm η) i • Pi.basisFun ℂ (Fin (kirovGenus Y)) i))) := by
      dsimp [latticeBridgeInv]
      simp_rw [← smul_eq_mul, ← map_smul, ← map_sum]
    rw [h_eval]
    have h_sum : ∑ i, ((ambientIso Y).symm η) i • Pi.basisFun ℂ (Fin (kirovGenus Y)) i = (ambientIso Y).symm η := by
      have h_decomp := (Pi.basisFun ℂ (Fin (kirovGenus Y))).sum_repr ((ambientIso Y).symm η)
      simp only [Pi.basisFun_repr] at h_decomp
      exact h_decomp
    rw [h_sum]
    have h_inv : bridgeKDFormEquiv.symm ((ambientIso Y) ((ambientIso Y).symm η)) = (jacobianBasis Y) j := by
      dsimp [η]
      simp only [LinearEquiv.apply_symm_apply, LinearEquiv.symm_apply_apply]
    rw [h_inv]
  exact (LinearEquiv.injective (jacobianBasis Y).dualBasis.equivFun.symm) h_eq

/-- The canonical linear isomorphism between Kirov's period coordinates and our basis coordinates. -/
noncomputable def latticeBridgeEquiv (Y : Type*) [TopologicalSpace Y] [T2Space Y] [CompactSpace Y]
    [ConnectedSpace Y] [ChartedSpace ℂ Y] [IsManifold 𝓘(ℂ) ω Y] [Nonempty Y] :
    (Fin (kirovGenus Y) → ℂ) ≃ₗ[ℂ] (Fin (genus Y) → ℂ) where
...
lemma ambientPhi_ambientPullback_periodVec_of_cycle {X Y : Type*}
    ...
    (h_pushforward : ∑ i, coeffs i • Jacobians.periodVec (f ∘ loops i) =
        (Jacobians.degreeFiber f hf) • Jacobians.periodVec δ) :
    Jacobians.ambientPhi (gX := kirovGenus X) (gY := kirovGenus Y) f hf
      ...
      (Jacobians.degreeFiber f hf) • Jacobians.periodVec δ := by
...
theorem exists_preimageCycle_sheets_eq_degree {X Y : Type*}
    ...
    ∃ c : Jacobians.PreimageCycle f hf δ, c.sheets = Jacobians.degreeFiber f hf := by
  ...
  · refine ⟨{ n := 0, loops := Fin.elim0, loops_smooth := fun i => i.elim0,
    ...
    · simp
    · show (0 : ℕ) = Jacobians.degreeFiber f hf
      have hcm : Jacobians.Discharge.IsConstantMap f := hconst
      rw [Jacobians.degreeFiber]
  ...
  · obtain ⟨c, y₀, hy₀, hsheets⟩ :=
    ...
    show c.sheets = Jacobians.degreeFiber f hf
    rw [hsheets, show Jacobians.degreeFiber f hf = Jacobians.degreeFiber f hf from rfl,
...
private lemma ambientPhi_ambientPullback_eq_on_lattice {X Y : Type*}
    ...
    Jacobians.ambientPhi (gX := kirovGenus X) (gY := kirovGenus Y) f hf
      ...
      (Jacobians.degreeFiber f hf) • v := by
...
theorem ambientPhi_ambientPullback_eq {X Y : Type*}
    ...
    Jacobians.ambientPhi (gX := kirovGenus X) (gY := kirovGenus Y) f hf
      ...
      (Jacobians.degreeFiber f hf) • y := by
  classical
  ...
  have hlat : ∀ i, Φ (b i) = (Jacobians.degreeFiber f hf) • b i := by
    intro i
    have hmem : b i ∈ Jacobians.truePeriodLattice Y := by
      ...
      exact truePeriodLattice_le_periodLatticeInBasis Y (Module.Basis.ofZLatticeBasis ℝ Λ b₀ i) h_in
