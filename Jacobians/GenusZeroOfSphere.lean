...
* **`S²` is simply connected** — now *unconditional* in the repo
  (`Jacobians.SphereSimplyConnected` proves `twoOpenVanKampen_holds`, giving the global instance
...
(`Jacobians.SphereSimplyConnected`, `twoOpenVanKampen_holds`). Composing with
