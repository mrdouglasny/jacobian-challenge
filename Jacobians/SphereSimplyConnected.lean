...
`Jacobians/DegreeOneSphere.lean`) and its transport helper
...
open Set OnePoint unitInterval Path Fin
...
that each conjugated loop `c k .castSucc · F k · c k .succ⁻¹` is the identity at `o`, the concatenation of the
...
theorem telescope {o : Z} :
    forall {n : ℕ} (q : Fin (n + 1) -> Z) (c : (k : Fin (n + 1)) -> Q o (q k))
...
theorem nullhomotopic_of_mem_isSimplyConnected {W : Set Z} (hW : IsSimplyConnected W) {o : Z}
    (L : Path o o) (hL : forall t, L t ∈ W) : Path.Homotopic L (Path.refl o) := by
...
theorem exists_lebesgue_subdivision {γ : I -> Z} (hγ : Continuous γ) {U V : Set Z}
    ...
    ∃ (n : ℕ) (t : Fin (n + 1) -> I), t 0 = 0 ∧ t (last n) = 1 ∧
      forall k : Fin n, (γ '' (Set.uIcc (t k.castSucc) (t k.succ)) ⊆ U) ∨
  ...
  have hcopen : forall b, IsOpen (c b) := by
  ...
  · -- Per-arc: the closed piece lies in a single chart.
    ...
    have hval_of : forall x : ℝ, 0 ≤ x -> x ≤ 1 ->
...
theorem loop_at_overlap_nullhomotopic {U V : Set Z} (hU : IsOpen U) (hV : IsOpen V)
  ...
  -- the endpoint `q j`.
  have hspoke_exists : forall j : Fin (n + 1), ∃ p : Path o (q j),
      forall s, (q j ∈ U -> p s ∈ U) ∧ (q j ∈ V -> p s ∈ V) := by
    intro j
  ...
  have hcert : forall j : Fin (n + 1), forall s,
  ...
  have harc : forall k : Fin n,
    ...
    have main : forall W : Set Z, IsSimplyConnected W -> range (F k) ⊆ W ->
        (q k.castSucc ∈ W) -> (q k.succ ∈ W) ->
        (forall j, q j ∈ W -> forall s, spoke j s ∈ W) ->
        Path.Homotopic ((spoke k.castSucc).trans ((F k).trans (spoke k.succ).symm))
          (Path.refl o) := by
...
theorem twoOpenVanKampen_holds : TwoOpenVanKampen := by
...
/-- **The Euclidean `2`-sphere `S² ⊆ ℝ³` is simply connected** — unconditionally. -/
instance : SimplyConnectedSpace (Metric.sphere (0 : EuclideanSpace ℝ (Fin 3)) 1) :=
  simplyConnectedSpace_sphere_of_vanKampen VanKampen.twoOpenVanKampen_holds
