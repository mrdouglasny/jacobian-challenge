...
**Active project axioms: 54** — all **54** in our own modules. The vendored
...
remain.
Then → **54** by proving `PlaneCurve.instT2Space`: `PlaneCurve H` is a subtype
of the projectivization quotient, and `projectivization_t2Space` supplies the
ambient `T2Space` instance inherited by the subtype. The remaining `PlaneCurve`
topology/manifold axioms are compactness, connectedness, nonemptiness, charted
space, and manifold compatibility.
...
- **Class 2 — form or proof not yet clear** (42 axioms). Either the Lean
...
| 2c — atlas / structure | 23 | curve-specific chart constructions | real but unverified |
...
| `PlaneCurve`: `instNonempty` + 4 manifold/topology instances (`instCompactSpace`/`instConnectedSpace`/`instChartedSpace`/`instIsManifold`) + 3 affine props (type + `instTopologicalSpace` discharged Phase-3 Tier-1; `instT2Space` is now proved; `instNonempty` reverted in review) | `ProjectiveCurve/PlaneCurve.lean` | 8 |
...
#   prints 54 — the vendored Kirov subtree is now axiom-free, so 54 is the total.
...
LEAN
lake env lean /tmp/axcount.lean   # → project axioms (non-vendor): 54
