...
**Active project axioms: 13** — all **13** in our own modules (the
...
|-------|-----------|-----------|
| `AX_AbelSupset` | `Axioms/AbelTheorem.lean:113` | Forster §20.7 (Abel's theorem, "only if"); Mumford I §II.3; Griffiths–Harris Ch. 2 §2. **2026-06-12 split-flip remainder**: the ⊇/AJ half of the former `AX_AbelTheorem` (`PrincipalDivisors ≤ ker abelJacobiDiv`); strictly weaker than the DT-re-vetted (2026-06-09 SATISFIABLE/FAITHFUL) full statement, so vetting is inherited. Discharge route: Liouville / symmetric-product (`docs/planning/ABEL_SUPSET_LIOUVILLE_ROUTE.md`). `AX_AbelTheorem` itself is now a THEOREM (⊆ = §20 engine + E6 adapter; ⊇-deg = `deg_divisor_eq_zero`) |
...
LEAN
lake env lean /tmp/axcount.lean   # → project axioms (non-vendor): 13
