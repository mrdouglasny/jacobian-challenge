...
(`RiemannSurface/QuotientCoveringPi1.lean`: Mathlib homotopy-lifting +
deck-translation equivariance, injective by projecting a nullhomotopy of the
lift over simply connected `V`, surjective by straight-line loops) is
specialized at g = 1 in `ProjectiveCurve/Elliptic/H1Basis.lean`: the Hurewicz
classes of the concrete loops compute to `aLoop ↦ ω₁`, `bLoop ↦ ω₂`,
`bLoopRev ↦ −ω₂` by explicit lifts, the oriented lattice ℤ-basis `{ω₁, ±ω₂}`
transports to `ellipticH1Basis` with `ellipticH1Basis_eq_loops`, and feeding
the #225 conditional corollary `ellipticPeriodCycleBasisOfH1` (Hodge fields
R1/R2 already proven via the boundary-word datum) yields
`ellipticPeriodCycleBasis : PeriodCycleBasis (Elliptic ω₁ ω₂ h) 0` —
**kernel-verified standard-3** (`propext, Classical.choice, Quot.sound`; no
`AX_PeriodCycleBasis`, no `AX_Elliptic_H1_symplectic`). This is the FIRST
fully unconditional instantiation of the period-cycle-basis content on any
positive-genus curve; net −1, 12 → 11, challenge-critical unchanged at 1.
Before that the
**ABEL ⊇ FLIP**, 2026-06-12 (SUP lane, `feat/abel-supset`): **`AX_AbelSupset`
is now a THEOREM** — the Liouville / symmetric-product route executed in full
(S1–S7 of `docs/planning/SUP_ROUTE.md`): the Jacobi pencil map
`Φ(y) = AJ(f⁻¹(y))` (`fiberAJ`) is smooth at regular values via local
holomorphic sections of `toP1 f` (Wallace local inverse + the proven
`AX_ofCurve_contMDiff`, `AbelSupsetSections.lean`), continuous everywhere and
`MDifferentiable` across the branch values (kfold cluster decomposition +
sumset-neighborhood continuity + a generic manifold-valued removable
singularity, `AbelSupsetPencil.lean`), and constant by the lattice-covering
lift over the simply connected `ℙ¹` (Kirov van-Kampen capstone) + Liouville
(`AbelSupsetLiouville.lean`); `abel_supset_of_fiberAJConstancy` then closes
the statement verbatim in place. Consequently **`AX_AbelTheorem` and
`ofCurve_inj` are now standard-3 + `AX_PeriodCycleBasis` only** —
challenge-critical drops **2 → 1** (`AX_PeriodCycleBasis` alone); net −1,
13 → 12. Kernel log: `docs/planning/ABEL_FLIP_VERIFICATION.log` (SUP section).
Before that the
**ABEL SPLIT-FLIP**, 2026-06-12 (feat/abel-flip lane): `AX_AbelTheorem` is now a
**theorem** — the ⊆ direction (deg-0 Abel–Jacobi kernel ≤ principal divisors) is
PROVEN via the unconditional Forster §20 weak-solution engine
(`exists_meromorphic_of_zeroPeriodChain'`, port-side E-block) through the E6
adapter (`Jacobians/Bridge/AbelEngineAdapter.lean`: port `smoothPath` arcs +
closed-loop smooth representatives of the pinned cycle basis + `bridgeKDFormEquiv`
form transport + the port identity theorem for divisor faithfulness;
`Bridge.abel_subset` is standard-3 + `AX_PeriodCycleBasis` only), and the degree
half of ⊇ is the proven `deg_divisor_eq_zero`; the remaining ⊇ Abel–Jacobi half
is the strictly-smaller named remainder axiom **`AX_AbelSupset`**
(`PrincipalDivisors X ≤ (abelJacobiDiv X).ker`, Liouville route
`docs/planning/ABEL_SUPSET_LIOUVILLE_ROUTE.md`) — net 0 in count (swap within
Class 1), challenge-critical stays **2** by honest counting
(`AX_PeriodCycleBasis`, `AX_AbelSupset`); kernel evidence:
`docs/planning/ABEL_FLIP_VERIFICATION.log`. Before that the
**LINE-BUNDLE STUB RETIREMENT**, 2026-06-12, de-opaqued the last two opaque
type stubs `LineBundle` + `LineBundle.ofDivisor` to real `def`s (the
divisor-indexed tag structure — the only exposed interface was always
`H⁰ = riemannRochSpace D` / `H¹ = Layer3.H1coh D`, real constructions
depending on `D` alone; consumers elaborate unchanged, the #201
`canonicalDivisor` de-opaque pattern), so `AX_RiemannRoch` /
`AX_SerreDuality` are now **fully standard-3, statements included** — the
RR/Serre tower carries no project axiom in any form; net −2, 15 → 13,
challenge-critical unchanged. Before that the
...
proven ∃-K Serre package): the T-lane frame-trace wall
(`FrameTrace.exists_canonicalData_residueAtom`, the canonical `ω₀ = df` datum
satisfies its own residue atom — Miranda §VIII.3 trace route, sorry-free) fed
the #194 genus split and the #193 capstone, replacing the port's ∀-cover
keystone sorry `exists_serreDualityData` with the PROVEN ∃-cover keystone
`exists_serreDualityData_cover` and re-pinning `exists_riemannRoch_divisor`
onto the exhibited cover; `riemannRochL3`, `serreDualityL3`, `h0_canonical_L3`,
`canonicalDivisor_deg_L3` are now **standard-3**, and `AX_RiemannRoch` /
`AX_SerreDuality` drop to standard-3 + the two opaque `LineBundle` type stubs
appearing in their own statements — the Layer-3 Riemann-Roch closure is
axiom-free over the port; net −3, 21 → 18; #30
discharged `AX_pushforwardAmbient_preserves_lattice` to a theorem via the
developing-value naturality route — `pullbackOneForm_isPullbackCoeffRel`
unwinds the Kirov bridge to the pointwise `mfderiv` pullback law, the
span-induction engine `developingValue_comp_of_isPullbackCoeffRel` gives
`∫_γ f^*ω = ∫_{f∘γ} ω`, and `devVal_loop_mem_periodLatticeInBasis_any`
lands the image loop's period vector in the target lattice, 2026-06-11,
headlines `pushforward`/`pushforward_contMDiff`/`pushforward_id_apply`/
`pushforward_comp_apply` now standard-3 + `AX_PeriodCycleBasis` only,
net −1, 22 → 21; PR #183
...
- **Class 3 — Layer-3 primitives** (0 axioms). Standard cohomology
  primitives introduced to prove earlier table entries as theorems while
  keeping the remaining trust boundary explicit. **Both DISCHARGED by the
  keystone flip 2026-06-11** (see Recently discharged). (The 2
  Riemann-bilinear primitives `AX_RBR1`/`AX_RBR2` were merged into
  `AX_PeriodCycleBasis` in D1, 2026-06-10.)
...
| 2a — data-existence | 1 | "this function/object exists with spec S"; `AX_curve_image_subgroup_isOpen` (AK, local Jacobi inversion) **DISCHARGED 2026-06-16 PR #255** (@daouid — now theorem, std-3 ⇒ `ofCurve_isJacobian` axiom-free); the 2 `LineBundle` type stubs DISCHARGED 2026-06-12 (now real `def`s) | spec needs review |
...
|-------|-----------|-----------|
| `AX_AbelSupset` | ✅ **DISCHARGED 2026-06-12** (SUP lane) — now a `theorem`, statement verbatim at `Axioms/AbelTheorem.lean` (see Recently discharged) | Forster §20.7 (Abel's theorem, "only if"); Mumford I §II.3; Griffiths–Harris Ch. 2 §2. *(Pre-discharge record:)* the 2026-06-12 split-flip remainder, strictly weaker than the DT-re-vetted (2026-06-09 SATISFIABLE/FAITHFUL) full statement. Discharged along exactly the planned Liouville / symmetric-product route (`docs/planning/ABEL_SUPSET_LIOUVILLE_ROUTE.md`) |
...
| `AX_curve_generates_jacobian` | ✅ **DISCHARGED 2026-06-14** — retired; now the theorem `curve_generates_jacobian`, proved from AK (`AX_curve_image_subgroup_isOpen`) via open-subgroup-of-connected-group (AK itself discharged 2026-06-16 PR #255, so this is now **transitively axiom-free**) | Mumford *Curves & their Jacobians*; Milne *AV* §I |
| `AX_torus_uniformization` (A1) | ✅ **OUT of every headline closure 2026-06-14** (escape-hatch `bc3a115`): `TorusSelfAlbanesePresentation` is now a `class`, so the target torus carries it as a typeclass parameter. `ofCurve_isJacobian` / `isJacobian_iso_jacobian` `#print axioms` = **std-3** (AK discharged 2026-06-16 PR #255); `isJacobian_unique` = std-3 (**axiom-free**). A1 stays *declared* (NO global instance) for the future abstract-torus generality file; satisfiable witness `ℂ^m/Λ` | Birkenhake–Lange Ch. 1 |
| `AX_curve_image_subgroup_isOpen` (AK) | ✅ **DISCHARGED 2026-06-16 PR #255** (@daouid) — now the theorem `RiemannSurface.curve_image_subgroup_isOpen` (`AlbaneseOpen.lean`), `#print axioms` = std-3; assembles the pre-existing IFT local Jacobi inversion (`exists_jacobiMap_map_nhds`) + a chart-FTC identity + Kirov's open lattice quotient (`ZLatticeQuotient.isOpenMap_mk`). `ofCurve_isJacobian` now axiom-free; kernel axioms 8→7. | local Jacobi inversion, Forster 21.3 |
...
| `LineBundle`, `LineBundle.ofDivisor` (2) | ✅ **DISCHARGED 2026-06-12 (stub retirement)** — real `def`s at `RiemannSurface/Cohomology/LineBundleBasic.lean` (see Recently discharged) | were line-bundle **type stubs**; now the divisor-indexed tag structure + its canonical inhabitant (the only exposed interface was always `H⁰`/`H¹`, real over `riemannRochSpace D` / `Layer3.H1coh D`; consumers elaborate unchanged). `canonicalDivisor` was already **DISCHARGED 2026-06-11 (keystone flip)**; the `Divisor` triple and `PrincipalDivisors` were already discharged. |
| Layer 3 Phase-B cohomology scaffold: `h1coh_zero_finrank`, `serreDuality_equiv` — **both DISCHARGED 2026-06-11 (keystone flip)**, statements verbatim (**`H1coh`+3 instances and `cohomologyLES` DISCHARGED 2026-06-10** — real Čech cohomology of the chart-disk cover via the Kirov Dolbeault port, `Layer3/CechH1Bridge.lean`, `#print axioms` standard-3) | `Layer3/Cohomology.lean` | Formerly the axiomatic remainder of the cohomology hybrid for the real `H¹(X,O(D))`; now theorems via the FlipPrep composition over the T-lane frame-trace wall (see Recently discharged). Originally Gemini deep-think vetted 2026-06-08/09 (`DT`, SATISFIABLE/FAITHFUL); the port-side interfaces backing the discharge were separately vetted Standard 2026-06-10 (A2, `docs/planning/PHASE_D_TYPE_ALIGNMENT.md`); references Forster §§16–17. Riemann-Roch and the Serre dimension identity are theorems in the same file — now **standard-3**. |
...
| Elliptic witness (`AX_Elliptic_H1_symplectic`) — ✅ **DELETED/DISCHARGED 2026-06-12 (EP lane)**: `π₁(ℂ/Λ) ≅ Λ` landed (`RiemannSurface/QuotientCoveringPi1.lean`), the H₁ ℤ-basis on the A/B classes + the boundary-word Hodge fields give the proven witness `ellipticCycleBasis`/`ellipticPeriodCycleBasis` in `Elliptic/H1Basis.lean`, kernel-verified **standard-3** (not even `AX_PeriodCycleBasis`). `AX_Elliptic_aLoop_analytic` (PR #86) and `AX_Elliptic_bLoop_analytic` (2026-06-07) were already theorems. | `…/Elliptic/H1Basis.lean` | 0 |
...
| `AX_curve_image_subgroup_isOpen` (AK) | ✅ **DISCHARGED 2026-06-16 PR #255** (@daouid) — now theorem `RiemannSurface.curve_image_subgroup_isOpen` (`AlbaneseOpen.lean`), std-3; `ofCurve_isJacobian` axiom-free | Forster 21.3 |
...
|-----------|----------------|----------------|
| `AX_Elliptic_H1_symplectic` *(2026-06-12, EP lane — the g = 1 cycle-basis witness)* | statement (`PeriodCycleBasis (Elliptic ω₁ ω₂ h) 0`) now PROVEN and the axiom deleted: covering-space `π₁(ℂ ⧸ Λ) ≅ Λ` + `H₁(ℂ ⧸ Λ) ≅+ Λ` (`h1EquivLattice`, lift-endpoint homomorphism; injectivity from simply connected `ℂ` by projecting a nullhomotopy of the lift, surjectivity from straight-line loops), Hurewicz classes of the concrete loops computed by explicit lifts (`aLoop ↦ ω₁`, `bLoop ↦ ω₂`, `bLoopRev ↦ −ω₂`), oriented lattice basis `{ω₁, ±ω₂}` transported to `ellipticH1Basis` aligned with `ellipticLoops`, fed into the #225 conditional corollary `ellipticPeriodCycleBasisOfH1` (Hodge fields R1/R2 proven, boundary-word datum). `#print axioms ellipticPeriodCycleBasis` = **standard-3** — the first fully unconditional positive-genus instantiation of the `AX_PeriodCycleBasis` content | `RiemannSurface/QuotientCoveringPi1.lean` (general `V ⧸ Λ` engine), `ProjectiveCurve/Elliptic/H1Basis.lean` (g = 1 witness) |
| `AX_AbelSupset` *(2026-06-12, ABEL ⊇ FLIP, SUP lane `feat/abel-supset` — challenge-critical)* | statement verbatim, now a `theorem`: `abel_supset_of_fiberAJConstancy (fiberAJConstancy X)`. The Liouville / symmetric-product route, all rungs proven this lane: **S1** kernel converse + reduction (`AbelSupsetPlumbing.lean`); **S2** `fiberDivisor` + `divisor = fiber(0) − fiber(∞)`; **S3** the Jacobi pencil map `fiberAJ` + the constancy reduction; **S4a** finite `branchValues` + regular fibers unramified; **S4b** local holomorphic sections through unramified fiber points (Wallace `IsHolomorphicAt.localInverse` + Mathlib `analyticAt_localInverse`; eventual fiber-divisor trivialization `fiberDivisor y = ∑ᵢ of (sᵢ y)`, `AbelSupsetSections.lean`); **S4c** `contMDiffAt_fiberAJ` at regular values (proven `AX_ofCurve_contMDiff` ∘ sections, `ContMDiffAt.sum` over the Jacobian `LieAddGroup`); **S5** `mdifferentiable_fiberAJ` on ALL of `ℙ¹` (kfold cluster decomposition at any value + sumset-neighborhood continuity `exists_nhds_zero_finsetSum_mem` + the generic manifold-valued removable singularity `mdifferentiableAt_of_continuousAt_of_eventually_mdifferentiableAt`, `AbelSupsetPencil.lean`); **S6** `fiberAJConstancy` (simply connected `ℙ¹` via the Kirov van-Kampen capstone, continuous lift through the lattice covering via Mathlib `IsCoveringMap.existsUnique_continuousMap_lifts`, lift holomorphic via `ComplexTorus.mdifferentiable_lift_of_mdifferentiable` — local deck-constancy by discrete-subgroup isolation — then `MDifferentiable.apply_eq_of_compactSpace`, `AbelSupsetLiouville.lean`). No residue theorem, no Stokes. `#print axioms`: `AX_AbelSupset`, `AX_AbelTheorem`, `Jacobian.ofCurve_inj` = standard-3 + `AX_PeriodCycleBasis`; the S4b/S5 engine lemmas standard-3. Net −1 (13 → 12), challenge-critical 2 → **1** (`AX_PeriodCycleBasis` alone). Kernel log: `docs/planning/ABEL_FLIP_VERIFICATION.log` (SUP section) | `Axioms/AbelTheorem.lean` (the theorem); `RiemannSurface/AbelSupsetPlumbing.lean`, `AbelSupsetSections.lean`, `AbelSupsetPencil.lean`, `AbelSupsetLiouville.lean` (the ladder) |
| `AX_AbelTheorem` *(2026-06-12, ABEL SPLIT-FLIP, feat/abel-flip lane — challenge-critical)* | statement verbatim, now a `theorem` (`le_antisymm`). **⊆ (the hard half, ker⊓ker ≤ Principal): PROVEN** — A-block plumbing (`AbelPlumbing.lean`, PR #211) + the E-block unconditional Forster §20 weak-solution engine (`exists_meromorphic_of_zeroPeriodChain'`, port `AbelSubsetEngineArc.lean`) + the new E6 adapter `Bridge/AbelEngineAdapter.lean` (chain = port `smoothPath` basepoint arcs weighted by `equivFinsupp D` − re-expanded pinned-loop combination; per-arc reference-path mismatch absorbed as the developing value of a closed comparison loop, a `periodLatticeInBasis` vector; periods vanish on the bridged `jacobianBasis`, extended to all port forms by E1 spanning; engine divisor output pulled back via the port identity theorem `orderW_ne_top_of_exists` + Layer3 order dictionaries). **⊇ degree half: PROVEN** (`principalDivisors_le_deg_ker` from `deg_divisor_eq_zero`). **⊇ AJ half: split out** as the strictly-smaller `AX_AbelSupset` (Class 1, above). `abelJacobiDiv` moved to the base file `Axioms/AbelJacobiDivDef.lean` (no import cycle). `#print axioms`: `Bridge.abel_subset` + `zeroPeriodChainSolvability_of_engine` = standard-3 + `AX_PeriodCycleBasis`; `mem_principalDivisors_of_port_div` = standard-3; `AX_AbelTheorem` / `ofCurve_inj` = standard-3 + `AX_PeriodCycleBasis` + `AX_AbelSupset`. Net 0 (13 → 13, axiom swap); challenge-critical stays 2 (honest split count; full flip would be 1). Kernel log: `docs/planning/ABEL_FLIP_VERIFICATION.log` | `Axioms/AbelTheorem.lean` (theorem + remainder axiom); `Bridge/AbelEngineAdapter.lean` (new); `Axioms/AbelJacobiDivDef.lean` (new) |
| `LineBundle`, `LineBundle.ofDivisor` *(2026-06-12, stub retirement — handover Package 5)* | the last two opaque type stubs de-opaqued to real `def`s, the #201 `canonicalDivisor` pattern (fully-qualified names kept, every consumer elaborates unchanged — verified per-file). `LineBundle D` is now the divisor-indexed **tag structure**: on a compact Riemann surface `𝒪(D)` is determined up to isomorphism by `D`, and the development only ever exposes its cohomology — `H⁰ = riemannRochSpace D` and `H¹ = Layer3.H1coh D`, both real constructions depending on `D` alone — so the bundle object carries no further data; a parameterized `structure` (not `PUnit`) keeps `LineBundle D₁`/`LineBundle D₂` definitionally distinct so consumers cannot silently cross divisors. `LineBundle.ofDivisor D := ⟨⟩` is its canonical inhabitant. Consequence: `AX_RiemannRoch` (#38) and `AX_SerreDuality` (#37) are now **fully standard-3, statements included** — the entire RR/Serre tower carries no project axiom in any form. `#print axioms`: `AX_RiemannRoch`, `AX_SerreDuality`, `LineBundle.ofDivisor`, `H0_equiv_riemannRochSpace` all exactly `[propext, Classical.choice, Quot.sound]`; no `sorryAx`. Net −2 (15 → 13), challenge-critical unchanged. Closes #65, #70 (and the stubs-only residue of #37, #38). | `RiemannSurface/Cohomology/LineBundleBasic.lean` |
