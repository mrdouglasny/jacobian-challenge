...
**Active project axioms: 21** — all **21** in our own modules (#30
...
| `AX_pushforward_pullback` | ✅ **DISCHARGED 2026-06-11** — now a theorem: reduces to the coordinate-bridge compatibilities `pullbackAmbientLinear_eq_compat` and `pushforwardAmbientLinear_eq_compat`, which compose to the ambient identity `ambientPhi ∘ ambientPullbackJac = degreeFiber • id`. Since Kirov's ambient identity `ambientPhi_ambientPullback_eq` has been made sorry-free via our new real basis construction, the descended identity holds sorry-free. `#print axioms`: theorem = standard-3 + `AX_PeriodCycleBasis` + the two `latticeBridge` bridge axioms. |
| `AX_pushforwardAmbient_preserves_lattice` | ✅ **DISCHARGED 2026-06-11** (#30) — now a theorem: representative-loop induction over the developing-value naturality engine (`DevelopingNaturality.lean` + `LoopLattice.lean`, axiom-free; `pullbackOneForm_isPullbackCoeffRel` standard-3); headline closures drop to standard-3 + `AX_PeriodCycleBasis`. *(Pre-discharge vetting, retained for the record:)* **DT-vetted 2026-06-09: SATISFIABLE/FAITHFUL** — `pushforwardAmbientLinear` is the dual of the REAL `pullbackOneForm` (direction verified in code), so this is classical Albanese naturality `∫_{f_*γ}ω = ∫_γ f^*ω` + integrality of `f_*γ`; basepoint-free as a subset. (A DT direction-flag was a prose artifact; resolved against the defs.) |
| `AX_pullbackAmbient_preserves_lattice` | ✅ **DISCHARGED 2026-06-11** — now a theorem: proved using the sorry-free real basis of Kirov's period lattice. We map our sorry-free `IsZLattice` basis on `periodLatticeInBasis` via the coordinate bridge `latticeBridgeInv` into `truePeriodLattice`, establishing a sorry-free real basis of `truePeriodLattice` and bypassing Kirov's topological basis theorem. The lattice-preservation is then derived via Kirov's `ambientPullbackJac_preserves_truePeriodLattice` and `latticeBridge_truePeriodLattice_le` bridge axioms. `#print axioms`: theorem = standard-3 + `AX_PeriodCycleBasis` + the two `latticeBridge` bridge axioms. |
| `latticeBridge_truePeriodLattice_le` | `Bridge/KirovDolbeaultLattice.lean:247` | **DT-vetted 2026-06-11: SATISFIABLE/FAITHFUL** — Asserts that Kirov's true period lattice maps into our coordinate period lattice under `latticeBridge`. Satisfiable because Kirov's lattice is the $\mathbb{Z}$-span of period vectors of closed smooth loops, and `latticeBridge` maps the period vector of any closed smooth loop to its coordinate period vector in `periodLatticeInBasis` (using the homology Hurewicz map). Since the map is linear, the image of the $\mathbb{Z}$-span is contained in the target submodule. |
| `truePeriodLattice_le_periodLatticeInBasis` | `Bridge/KirovDolbeaultLattice.lean:253` | **DT-vetted 2026-06-11: SATISFIABLE/FAITHFUL** — Asserts that our coordinate period lattice maps into Kirov's true period lattice under `latticeBridgeInv`. Satisfiable because every element in `periodLatticeInBasis` is the coordinate period vector of some homology class $h \in H_1(X; \mathbb{Z})$. By Hurewicz, $h$ is represented by a closed loop, which can be smoothed to a closed smooth loop. Its period vector under `latticeBridgeInv` is exactly Kirov's period vector of this loop, which lies in `truePeriodLattice` by definition. |
...
|-----------|----------------|----------------|
| `AX_pullbackAmbient_preserves_lattice` *(2026-06-11)* | proved using the sorry-free real basis of Kirov's period lattice. We map our sorry-free `IsZLattice` basis on `periodLatticeInBasis` via the coordinate bridge `latticeBridgeInv` into `truePeriodLattice`, establishing a sorry-free real basis of `truePeriodLattice` and bypassing Kirov's topological basis theorem. The lattice-preservation is then derived via Kirov's `ambientPullbackJac_preserves_truePeriodLattice` and `latticeBridge_truePeriodLattice_le` bridge axioms. `#print axioms`: theorem = standard-3 + `AX_PeriodCycleBasis` + the two `latticeBridge` bridge axioms. | `Bridge/KirovDolbeaultLattice.lean` + `Axioms/AbelJacobiMap.lean` |
| `AX_pushforward_pullback` *(2026-06-11)* | reduces to the coordinate-bridge compatibilities `pullbackAmbientLinear_eq_compat` and `pushforwardAmbientLinear_eq_compat`, which compose to the ambient identity `ambientPhi ∘ ambientPullbackJac = degreeFiber • id`. Since Kirov's ambient identity `ambientPhi_ambientPullback_eq` has been made sorry-free via our new real basis construction, the descended identity holds sorry-free. `#print axioms`: theorem = standard-3 + `AX_PeriodCycleBasis` + the two `latticeBridge` bridge axioms. | `Bridge/KirovDolbeaultLattice.lean` + `Axioms/AbelJacobiMap.lean` |
| `AX_pushforwardAmbient_preserves_lattice` *(#30, 2026-06-11)* | the developing-value naturality route (KIROV_ROUTE_IDEAS item 7 — span-induction idea credited to Kirov's tree, implementation ours): a lattice vector is the period vector of an `H1` class; every `H1` class is the class of a representative loop `γ`; dual-basis algebra turns `pushforwardAmbientLinear v` into the `γ`-integrals of `pullbackOneForm f (jacobianBasis Y j)`; the engine `developingValue_comp_of_isPullbackCoeffRel` (joint Lebesgue subdivision fine for X-charts of `γ` and Y-charts of `f∘γ`, primitive endpoint-difference transport across `f`) fed by `pullbackOneForm_isPullbackCoeffRel` (Kirov-bridge unwind: `sectionCoeff`/`localRep` → pointwise `mfderiv` pullback → chart-read `fderiv` factor by the chain rule) rewrites them as periods of the honest image loop `f∘γ`; `devVal_loop_mem_periodLatticeInBasis_any` (H1 functional at the loop's own basepoint + path-conjugation transport) lands that period vector in the target lattice at any basepoint. `#print axioms`: theorem = standard-3 + `AX_PeriodCycleBasis`; both engine modules axiom-free; no sorryAx. Net −1 (22 → 21); affected headlines `pushforward`/`_contMDiff`/`_id_apply`/`_comp_apply` drop to standard-3 + `AX_PeriodCycleBasis`. | `RiemannSurface/DevelopingNaturality.lean` + `RiemannSurface/LoopLattice.lean` (engine) + `Axioms/AbelJacobiMap.lean` (bridge unwind + the theorem) |
...
#   prints 21 — the vendored Kirov subtree is now axiom-free, so 21 is the total (incl. the 2 Layer-3 cohomology axioms after the Phase-D H1coh(+3 instances)+cohomologyLES discharges and the D1 merge of the 3 cycle-basis/RBR axioms into AX_PeriodCycleBasis; less the 5 discharged RR/Serre+H1, the 3 discharged period-cluster axioms, PlaneCurve.instChartedSpace #117 + instIsManifold #52, the 3 trace-cluster axioms #26/#27/#28, AX_ofCurve_contMDiff PR #179, the 7-axiom odd-atlas infinity-chart cluster PR #183, and AX_pushforwardAmbient_preserves_lattice #30).
...
LEAN
lake env lean /tmp/axcount.lean   # → project axioms (non-vendor): 21
