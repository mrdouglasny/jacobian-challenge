...
**Active project axioms: 17** — all **17** in our own modules (the
**GENUS-0 UNIFORMIZATION FLIP**, 2026-06-11, discharged the challenge-critical
`AX_genus_eq_zero_iff_homeo` to a theorem: forward leg via the keystone-backed
Riemann–Roch pole extraction — `h⁰((p)) = 2` at `g = 0` gives a degree-one
function, hence a biholomorphism to `ℙ¹`, then stereographic to `S²`
(`RiemannSurface/GenusZeroForward.lean`); backward leg via the S2-lane
`π₁(S²) = 1` + Liouville developing map (`Bridge/SphereGenusZero.lean`); the
challenge headline `genus_eq_zero_iff_homeo` is now **standard-3**; net −1,
18 → 17. Before that the
...
|------|------:|--------|-------|
| 1 — textbook-standard | 9 | classical theorems, citable | high |
...
| `AX_genus_eq_zero_iff_homeo` | ✅ **DISCHARGED 2026-06-11** — now a `theorem` (see Recently discharged); statement verbatim at `Axioms/Uniformization0.lean` | uniformization, genus 0 (Forster §27). Was DT-vetted 2026-06-09 SATISFIABLE/FAITHFUL; discharged along exactly the vetted routes (→ RR pole extraction, ← homeo-invariance of simple connectedness + Liouville) |
...
the abstract `genus_eq_zero_iff_homeo` is now standard-3, and the concrete
`genus ℙ¹ = 0` was already proven directly (see Recently discharged).
...
| `LineBundle`, `LineBundle.ofDivisor` (2) | `RiemannSurface/Cohomology/LineBundleBasic.lean` | line-bundle **type stubs**. `canonicalDivisor` **DISCHARGED 2026-06-11 (keystone flip)** — now a `noncomputable def`, the `Classical.choose` of the proven ∃-K Serre package `serreDuality_equiv_exists` (its `choose_spec` is the verbatim `serreDuality_equiv` statement). `H0` is `riemannRochSpace D` with inherited submodule instances; `H1` is now `Layer3.H1coh D` with inherited Layer-3 instances; the `Divisor` triple and `PrincipalDivisors` were already discharged. |
...
|-----------|----------------|----------------|
| `AX_genus_eq_zero_iff_homeo` *(2026-06-11, genus-0 uniformization flip — challenge-critical)* | statement verbatim, now a theorem. **Forward** (`g = 0 ⇒ X ≃ₜ S²`): the keystone-backed `riemannRoch` at a point divisor gives `h⁰((p)) = 2` at genus 0 (`h0_point_eq_two_of_genus_zero` — the dual term dies by `deg(K−(p)) = −3 < 0`); a non-constant `F ∈ L((p)) \ L(0)` has principal divisor exactly `(q) − (p)`, `q ≠ p` (degree theorem via `divisorOf_eq_pointSub_of_mem_point`); `toP1 F` hits `0` at `q` and `∞` at `p` so `F` is nonconstant, and `degreeOne_equiv_projectiveLine` (the exposed-equivalence refactor of `degreeOne_genus_zero`) upgrades it to a biholomorphism `X ≃ ℙ¹` (weighted-fiber-sum bijectivity + IFT inverse analyticity), composed with the stereographic `ℙ¹ ≃ₜ S²`. **Backward** (`X ≃ₜ S² ⇒ g = 0`): transport `SimplyConnectedSpace S²` (S2-lane: ported two-open van Kampen, `Topology/SphereSimplyConnected.lean`) across the homeomorphism, then every holomorphic 1-form has a global developing-map primitive, constant by Liouville (`genus_eq_zero_of_homeo_sphere_unconditional`). `#print axioms`: `AX_genus_eq_zero_iff_homeo`, the challenge headline `genus_eq_zero_iff_homeo`, and all forward-chain lemmas **standard-3**; no `sorryAx`. Net −1 (18 → 17), challenge-critical −1 (5 → 4). | `Axioms/Uniformization0.lean` (the theorem); forward: `RiemannSurface/GenusZeroForward.lean` (new) + `RiemannSurface/DegreeOneGenusZero.lean` (equiv exposed) + `RiemannSurface/Cohomology/RiemannRochAPI.lean`; backward: `Bridge/SphereGenusZero.lean` + `Topology/SphereSimplyConnected.lean` |
...
LEAN
lake env lean /tmp/axcount.lean   # → project axioms (non-vendor): 17
