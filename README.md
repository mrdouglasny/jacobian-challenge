...
**Categorical pin (beyond the API).** Buzzard's API characterizes the Jacobian *operationally* (functoriality + degree + Abel injectivity); it never states the Albanese **universal property** that pins `(Jac X, aj)` up to unique isomorphism. We add it as a compiling, cross-model-vetted statement — `Jacobians.IsJacobian` in [`Jacobians/UniversalProperty.lean`](Jacobians/UniversalProperty.lean): `aj : X → J` to a complex torus, universal among pointed holomorphic maps to complex tori. Vetted by Gemini + Codex (→ minimal hypotheses `AddGroup`, `T2Space`); elaborates against v4.30. **Categoricity PROVED (2026-06-05):** `ofCurve_isJacobian` — for genus > 0, Buzzard's concrete `Jacobian X` / `ofCurve x₀` *satisfy* `IsJacobian` (every pointed holomorphic `f : X → A` to a complex torus factors uniquely through `ofCurve` by a holomorphic group hom). Genuine `∃!` (existence via pullback-of-1-forms → period coordinates → lattice descent; factorization; uniqueness via curve-generates-Jacobian). Rests on 3 vetted textbook torus axioms (`AX_torus_oneforms_dualCover`, `AX_torus_self_albanese`, `AX_period_functoriality`) + `AX_curve_generates_jacobian` + the pre-existing period axioms, plus the now-**discharged** `AX_torus_descent_holo` (a `theorem` since 2026-06-06); `#print axioms`-clean (no `sorryAx`). This pins the Jacobian up to unique isomorphism — a stronger anti-degeneracy target than Buzzard's API requires.
...
- **1 data-level function-existence axiom** (`pushforwardOneForm`): has a construction plan in [`docs/construction-plans/`](docs/construction-plans/). *(`pathIntegralBasepointFunctional` was de-opaqued to a real `∫` def 2026-06-04; **`loopIntegralToH1` — the period/path-independence pairing — was discharged to a real `def` 2026-06-05** via the cycle basis: `cb.isBasis.constr ℤ (∮ over the analytic cycle-basis loops)`, the integrals being the genuine L0–L1 multi-chart line integral. `periodMap`/`ofCurve` now rest on real integrals + the cycle-basis axioms, not the opaque `loopIntegralToH1`; the lone residual is the trivial `AX_cycleBasisLoop_integrable` (cycle-basis loops are rectifiable). Full homotopy invariance deferred.)*
...
### Axiom landscape — by topic and difficulty (62 axioms)

The **62** remaining axioms (kernel-verified count, `AXIOM_AUDIT.md`) group into seven topics. Difficulty is the *discharge* difficulty, not the depth of the underlying mathematics: 🟢 mechanical / available in Mathlib today, 🟡 substantial but standard (a known Lean route exists), 🔴 research-grade — the genuine textbook theorems whose Lean proofs do not yet exist anywhere.
...
| **Torus / Albanese universal property** | 4 | 🟡 | `AX_torus_oneforms_dualCover`, `AX_torus_self_albanese`, `AX_period_functoriality`, `AX_curve_generates_jacobian`. Underpin the **proved** categoricity theorem `ofCurve_isJacobian`; all Gemini+Codex-vetted. *(`AX_torus_descent_holo` was the 5th — discharged to a theorem 2026-06-06.)* |
...
Counts are approximate because the audit bundles some tightly-coupled clusters (e.g. the 6-axiom `LineBundle` group) into one row; the **62** total is exact. Full per-axiom table with `file:line` and ratings: [`AXIOM_AUDIT.md`](AXIOM_AUDIT.md); per-axiom discharge plans below.
...
- [`Jacobians/Vendor/Kirov/`](Jacobians/Vendor/Kirov/) — six modules ported into our build under namespace `Jacobians.Vendor.Kirov.*` (`Genus`, `Montel.*`, `HolomorphicForms`, `LineIntegral`, `ChartedSpaceOfLocalHomeomorph`, `ZLatticeQuotient`), ~5,600 LOC total, with per-file Apache 2.0 attribution headers; mathematics unchanged. Kirov's two upstream `:= sorry` declarations (`genus_eq_zero_iff_homeo`, `ambientPhi_ambientPsi_eq`) were briefly named `axiom`s on import, then **deleted 2026-06-04** once confirmed unused — the vendored subtree is now axiom-free.
...
* **`loopIntegralToH1`** (the H₁-level period pairing) — **DISCHARGED to a real `def` 2026-06-05** (`RiemannSurface/LoopIntegral.lean`). The multi-chart `pathIntegralAnalyticArc` is now a real, partition-independent line integral (L0–L1: chart-cover partition, cotangent-cocycle integrand independence, segment/adjacency calculus — all proven, ~10 modules), and the period pairing is `cb.isBasis.constr ℤ (∮ over the analytic cycle-basis loops)`. The lone residual is `AX_cycleBasisLoop_integrable` (cycle-basis loops are rectifiable — trivial regularity). Full homotopy invariance (representative-independence for arbitrary loops) is a deferred faithfulness upgrade; the chart-local engine (`Bridge/ContourDeformation.lean`) + square subdivision (L2) are in place for it.
* **`pathIntegralBasepointFunctional`** in `Axioms/AbelJacobiMap.lean` — **de-opaqued to a real `def` 2026-06-04** (`:= Bridge.kirovBackedFunctional`, a genuine `∫`).
...
| | |
|---|---|
| Build | `lake build Jacobians` green (verified 2026-06-06, 8597 jobs); zero `sorry` in `Challenge.lean`, the core construction, the concrete-curve witnesses, and the S1–S7 1-form framework; **11 gap-layer `sorry`**, all outside the core (`Extensions/Hyperelliptic.lean` 6, `Extensions/AbelJacobi.lean` 4, `ProjectiveCurve/Hyperelliptic/AntiInvariance.lean` 1; `Bridge/KirovLineIntegral.lean` is now `sorry`-free — `chartLine_FTC` proven, the false full-path FTC deleted). Separately, the **anchor layer carries 14 intentional vetted-statement `sorry`** (`RiemannRochAPI` 8, `SerreDualityAPI` 3, `PluckerAPI` 3) — these are *deliberately* deferred proofs of Gemini-vetted RR/Serre/Plücker statements (see "Anchor APIs" above), a distinct category from the gap-layer holes. **62 axioms (all in our own modules; the vendored Kirov + Wallace subtrees are now axiom-free)**, kernel-verified and triaged in [`AXIOM_AUDIT.md`](AXIOM_AUDIT.md). Discharged 2026-06-04: the Phase-2 `Hyperelliptic` leaf instances; the **entire 6-axiom bridgePath cluster** (`Bridge/BridgePath.lean`, smooth path-connectedness, ~1450 LOC); and the **Phase-3 prerequisite-type batch** — the unified `Hyperelliptic` type became a real parity-dispatch `def` (+ `instTopologicalSpace`/`instChartedSpace`/`instIsManifold`/`oddEquiv`/`evenEquiv`; the carrier is **standard-3** via a `Decidable.casesOn` motive, with only the chart/manifold instances transporting the sound atlas axioms), and `PlaneCurve` became a faithful `Projectivization`-subtype `def` (+ topology). The remaining sheaf-cohomology cluster (`H1`/`LineBundle`) is kept as **honest classified axioms** with a machine-checkable faithfulness gate, while `H0` is now the concrete `riemannRochSpace D` with inherited submodule instances — [`SheafCohomologySpec.lean`](Jacobians/RiemannSurface/SheafCohomologySpec.lean), Buzzard's anti-degeneracy strategy one layer down (see [`docs/planning/PHASE_3_INFRA_PLAN.md`](docs/planning/PHASE_3_INFRA_PLAN.md)). *A 2026-06-04 review reverted two unsound/faithless discharges (`infinityInverseMap` arbitrary-root, `PlaneCurve.instNonempty` on a false axiom), hardened `PlaneCurveData`, deleted the 2 unused vendored Kirov handoff axioms (vendored subtree now axiom-free), and deleted the **false** dangling axiom `AX_pathIntegral_local_antiderivative` (a single-valued ℂ "FTC" — false on genus ≥ 1: forces a global primitive ⇒ zero periods), and **de-opaqued** `pathIntegralBasepointFunctional` to a real `∫` def (`:= Bridge.kirovBackedFunctional`) so `ofCurve` is a computed map. **2026-06-05: `loopIntegralToH1` — the period/path-independence pairing, the deepest analytic gap — was discharged to a real `def`** via the L0–L1 multi-chart line integral (chart-cocycle integrand independence + partition independence, all proven) extended over the analytic cycle basis (`cb.isBasis.constr`); `periodMap`/`ofCurve` no longer list it (historically count held at 64, residual = trivial `AX_cycleBasisLoop_integrable`; full homotopy invariance deferred). **2026-06-05: `PrincipalDivisors` was de-opaqued to the range of `divHom`, retiring one more axiom. Universal-property UP-0/UP-1 adds the torus-Albanese axioms (including a lift/descent E6 form). 2026-06-05: `H0` was de-opaqued to `riemannRochSpace D`, retiring three more axioms. **2026-06-06: `AX_torus_descent_holo` (the E6 form) was discharged to a real `theorem`** via the local-section route over Kirov's quotient local-homeomorphism API, taking the count to 62.*** `genus ℙ¹ = 0`, `genus Elliptic = 1`, Liouville Level 1, and pullback of holomorphic 1-forms are axiom-free. **2026-06-05: the headline anti-degeneracy witness `elliptic_ofCurve_injective` was proved** — the Abel–Jacobi map is genuinely injective on a real positive-genus curve (`Elliptic = ℂ/Λ`), a real computation through the period lattice (range ⊆ c·Λ), kernel-clean (standard-3 + cycle-basis axioms, not even `AX_PeriodLattice`). **2026-06-05: general positive-genus `AX_ofCurve_inj` is now DERIVED/retired as an axiom** in `Axioms/OfCurveInjective.lean`: `AX_Period_Triangle` gives basepoint independence, `AX_AbelTheorem` puts `(Q₁)-(Q₂)` in `PrincipalDivisors`, and `principal_imp_eq_of_genus_pos` forces `Q₁ = Q₂`. This is a one-for-one axiom swap (`-AX_ofCurve_inj +AX_Period_Triangle`), then the H0 de-opaque lowers the kernel count to **63**, and the E6 (`AX_torus_descent_holo`) discharge to **62**. Roadmap + dependency graph: [`docs/planning/GENERAL_ABEL_INJECTIVITY_STATUS.md`](docs/planning/GENERAL_ABEL_INJECTIVITY_STATUS.md) |
