...
|------|-----------|:---:|---|
| **1. Buzzard's challenge** — `Challenge.lean` + the construction | the interface Buzzard posed | — *(it **is** the challenge)* | **core `sorry`-free**; rests on the 41-axiom layer |
...
**The current method — the Layer-3 tower.** Part 2 is being executed by a *tower of reductions* ([`Jacobians/Layer3/`](Jacobians/Layer3/)): rather than fill the Forster anchor `sorry`s in place, the active route **axiomatizes a thin layer of standard cohomology / differential-geometry infrastructure and proves the axiom-table entries as theorems over it**, pushing the trust floor toward Mathlib one stratum at a time. Two strata so far — **Phase B** put RR + Serre over the 7-axiom cohomology scaffold (`H¹` + the divisor-addition LES + `h¹(𝒪)=g` + Serre iso; cross-model-vetted satisfiable/faithful, [#126](https://github.com/mrdouglasny/jacobian-challenge/issues/126)/[#131](https://github.com/mrdouglasny/jacobian-challenge/issues/131)), and **Phase C** (landed) **discharged the period/Hodge cluster** — `AX_RiemannBilinear`, `AX_PeriodLattice`, `instPeriodLatticeDiscrete` are now `theorem`s over two basis-free Riemann-bilinear primitives (`RBR1` isotropy + `RBR2` Hodge positivity, statement-vetted with the sign verified on the genus-1 torus) through an axiom-free period-lattice engine (net −1 axiom). This is **still Part 2** — *discharge the axiom layer* — not a fourth part: the tower is the *how*, complementary to (and now the primary execution of) the Forster-anchor route above.
...
Every axiom is a staging point with a citation and a discharge plan, classified in [`AXIOM_AUDIT.md`](AXIOM_AUDIT.md). They group into eight topics (counts sum to the **41** kernel-verified axioms; topic boundaries are soft — a few axioms could reasonably sit in an adjacent row); difficulty is the *discharge* difficulty (🟢 mechanical / available in Mathlib, 🟡 substantial but standard, 🔴 research-grade — a genuine textbook theorem with no existing Lean proof):
...
| Layer-3 scaffold: cohomology (`H1coh` + LES + `h¹(𝒪)=g` + Serre iso, 7 — RR/Serre proved over it, [#126](https://github.com/mrdouglasny/jacobian-challenge/issues/126)/[#131](https://github.com/mrdouglasny/jacobian-challenge/issues/131)) + period primitives (`RBR1`/`RBR2`, 2 — bilinear relations + period lattice proved over them, Phase C) | 9 | 🔴 |
