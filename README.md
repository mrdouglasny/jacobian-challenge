...
| **`sorry`s** | 0 in the core; 11 in out-of-scope extensions; 5 intentional anchor-statement deferrals |
...
- the **5 anchor-layer `sorry`s** — faithful, cross-model-vetted *statements* of Riemann–Roch (§16) and Serre duality (§17) in `Cohomology/RiemannRochAPI` (4) / `Cohomology/SerreDualityAPI` (1), with their **proofs deferred** (the Forster cohomological route). The keystone is now in place: `#113` proved the `riemannRoch` identity and `h⁰(K) = g`, and the `L(D)`-finiteness fact they rest on — formerly the axiom `riemannRochSpace_finiteDimensional` — is **itself now discharged**, proved elementarily ([#116](https://github.com/mrdouglasny/jacobian-challenge/issues/116)); the 5 anchor `sorry`s that remain are secondary corollaries plus Serre vanishing.
...
- **`sorry`s, in two honest categories** (the *core* — challenge API, Jacobian construction, curve witnesses, S1–S7 1-form framework — is `sorry`-free):
  ...
  - **5 anchor-layer** — *intentionally* deferred proofs of the vetted RR/Serre statements above (`Cohomology/RiemannRochAPI` 4, `Cohomology/SerreDualityAPI` 1). `PluckerAPI` is fully proved; `h⁰(0)=1` is proved axiom-free; `#113` proved `riemannRoch` and `h⁰(K)=g`; what remains are secondary corollaries plus Serre vanishing.
