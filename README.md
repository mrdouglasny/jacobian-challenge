...
- **2 data-level function-existence axioms** (`loopIntegralToH1`, `pushforwardOneForm`): each has a construction plan in [`docs/construction-plans/`](docs/construction-plans/). *(`pathIntegralBasepointFunctional` was de-opaqued to a real `∫` def 2026-06-04 — `:= Bridge.kirovBackedFunctional`; `ofCurve`'s only analytic axiom is now `loopIntegralToH1`.)*
