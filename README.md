...
For cross-project comparison: Kirov's off-the-shelf Claude Code baseline ([rkirov/jacobian-claude](https://github.com/rkirov/jacobian-claude)) — ~5K LOC after 3 days, self-rated ~10–15% toward full end-to-end, 10 named classical theorems still stubbed. Kirov disclaimed Riemann-surface domain knowledge ("I don't know any of the math (hope to learn it one day)") and did no review of Claude's output. Similar LOC, different frontier: we have every foundation def closed and every remaining axiom enumerated with a discharge plan; Kirov's is closer to a build-as-you-go sketch.
