...
| **`sorry`s** | 0 in the core; 11 in out-of-scope extensions; 2 intentional anchor-statement deferrals |
...
| **2. RR/Serre/sheaf subchallenge** — `RiemannSurface/Cohomology/` | discharge the deep axioms via one classical spine | **no** — the challenge *rests on* the axioms; this *proves* them | 2 anchor `sorry`s (RR 2 + Serre 0) |
...
- the **2 anchor-layer `sorry`s** — faithful, cross-model-vetted *statements* of Riemann–Roch (§16) and Serre duality (§17) in `Cohomology/RiemannRochAPI` (2) / `Cohomology/SerreDualityAPI` (0), with their **proofs deferred** (the Forster cohomological route). The keystone is now in place: `#113` proved the `riemannRoch` identity and `h⁰(K) = g`; the `L(D)`-finiteness fact they rest on — formerly the axiom `riemannRochSpace_finiteDimensional` — is **itself now discharged**, proved elementarily ([#116](https://github.com/mrdouglasny/jacobian-challenge/issues/116)); and **Serre vanishing is now proved** ([#120](https://github.com/mrdouglasny/jacobian-challenge/issues/120)) — `deg(div f) = 0` for a principal divisor (the degree theorem, proved axiom-free over the standard three) closes `h0_of_deg_gt` and `h1_eq_zero_of_deg_gt`. The 2 anchor `sorry`s that remain are secondary corollaries.
...
Every axiom is a staging point with a citation and a discharge plan, classified in [`AXIOM_AUDIT.md`](AXIOM_AUDIT.md). They group into seven topics (counts sum to the **41** kernel-verified axioms; topic boundaries are soft — a few axioms could reasonably sit in an adjacent row); difficulty is the *discharge* difficulty (🟢 mechanical / available in Mathlib, 🟡 substantial but standard, 🔴 research-grade — a genuine textbook theorem with no existing Lean proof):
...
- **`sorry`s, in two honest categories** (the *core* — challenge API, Jacobian construction, curve witnesses, S1–S7 1-form framework — is `sorry`-free):
  ...
  - **2 anchor-layer** — *intentionally* deferred proofs of the vetted RR/Serre statements above (`Cohomology/RiemannRochAPI` 2, `Cohomology/SerreDualityAPI` 0). `PluckerAPI` is fully proved; `h⁰(0)=1` is proved axiom-free; `#113` proved `riemannRoch` and `h⁰(K)=g`; `#120` proved Serre vanishing (`h0_of_deg_gt`, `h1_eq_zero_of_deg_gt`) via the degree theorem; what remains are secondary corollaries.
